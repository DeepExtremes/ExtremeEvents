...
@everywhere function era5varcube(varname, startyear, endyear;ext="an")
    ...
    timax = Dim{:time}(DateTime(startyear,1,1):Hour(1):DateTime(endyear,12,31,23,0,0))
...
map(to_run) do (varname,outvarname,ext,aggfun)
    c = era5varcube(varname,startyear, endyear,ext=ext)    
    ...
    savedataset(ds, path="/Net/Groups/BGI/scratch/mweynants/DeepExtremes/v3/ERA5Cube_$startyear.zarr",append=true, max_cache=3e8,writefac=0.1)
    nothing
end
