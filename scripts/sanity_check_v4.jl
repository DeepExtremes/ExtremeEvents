...
if occursin("/Users", pwd())
    path = "/Users/mweynants/BGI/Dheed_v4"
else
    path = "/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/"
end

obs = CSV.read("/Net/Groups/BGI/scratch/mweynants/DeepExtremes/v3/refEvents.csv", DataFrame)
...
labels_all = open_dataset(path * "mergedlabels_$(etrial)_1950_2023.zarr")
statevents = CSV.read(joinpath(path,"MergedEventStats_$(landonly)_int.csv"), DataFrame)
...
mkdir(joinpath(path,"fig/SanityCheck"))
for obs_event in 1:nrow(obs)
    println(obs_event)
    ...
    # === lon-lat view ===
    figT = Figure();
    axT = Axis(figT[1,1], title = obs[obs_event,:Event] * " in " * obs[obs_event, :Area] * "\n from " * Dates.format(obs[obs_event, :Start],"yyyy-mm-dd") * " to " * Dates.format(obs[obs_event, :End],"yyyy-mm-dd"),
            xlabel = "Longitude",
            ylabel = "Latitude");
    ...
    if !isempty(df)
        ...
        nl = step(labels_all.axes[:longitude])
    ...
    save(path * "fig/SanityCheck/plot_" * etrial * "_HistEvent_$(obs_event)_LatLon.png", figT)
    save(path * "fig/SanityCheck/plot_" * etrial * "_HistEvent_$(obs_event)_LatTime.png", figL)
    ...
    if !isempty(df) #any(df0.obs_event .== obs_event)
        ...
        if length(ulbls) <= 9
            ...
            cbar1 = Colorbar(fig[1,1][1,1], 
                label = "Events labels \n" * obs[obs_event,:Event] * " in " * obs[obs_event, :Area],
                colormap = cgrad(cols[1:8], 8, categorical=true),
            )
            cbar1.limits = (1,8)
            cbar1.ticks = ((1+(8-1)/8/2):((8-1)/8):(8), string.(ulbls[1:8]))
            cbar1.flipaxis = false
            cbar2 = Colorbar(fig[1,1][1,2], 
                colormap = cgrad(cols[9:end], 8, categorical=true),
            )
            cbar2.limits = (1,nlb-8)
            cbar2.ticks = ((1+(nlb-8-1)/(nlb-8)/2):((nlb-9)/(nlb-8)):(nlb-8), string.(ulbls[9:end]))
        ...
        for t in 1:n
            ...
            if any(ind)
                ...
                if typeof(lon) <: Vector
                    for L in lon
                    axt, h = labelplot!(axt, labels_all, periodt, lato, L, lblt; colormap = Makie.Categorical(cols[indexin(lblt, ulbls)]))
                    end
                else 
                    # if length(lblt) == 1
                    #     axt, h = labelplot!(axt, labels, periodt, lato, lono, lblt; color = cols[indexin(lblt, ulbls)])
                    # else
                        axt, h = labelplot!(axt, labels_all, periodt, lato, lono, lblt; colormap = Makie.Categorical(cols[indexin(lblt, ulbls)]))
                    # end
                end
            end
            # remove ticks
        ...
        save(path * "fig/SanityCheck/plot" * "_" * etrial * "_Event_$obs_event" * "_labels.png", fig) 
    end
end
...
tmp = df0 |> 
    ...
    (df -> rightjoin(df, DataFrame(obs_event = 1:maximum(obs.obs_event)), on = :obs_event => :obs_event)) |>
    (df -> transform(df, :label_start_time_etc_function => x -> replace(x, missing => 0))) |>
    (df -> rename!(df, Dict(:label_start_time_etc_function_function => "nevents",)))
tmp1 = df0 #|>
