...
@everywhere begin
    using Zarr, YAXArrays, Statistics#, SphericalConvolutions
    include("../src/detection.jl")
end

era = open_dataset("/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/ERA5Cube.zarr")
...
ranked_t = if ispath("/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/tmax_ranked.zarr")
    Cube("/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/tmax_ranked.zarr")    
else
    rescale(tair,"/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/tmax_ranked.zarr", multiplier = -1)
end

# PEI values of interest are in the lower range (P-E << 0), so we don't transform them before we normalize them
ranked_pei = if ispath("/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/pei_ranks.zarr")
    Cube("/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/pei_ranks.zarr")    
else
    rescale(peicube,"/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/pei_ranks.zarr")
end
