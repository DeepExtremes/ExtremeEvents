...
smoothed_t = smooth(ranked_t, "/Net/Groups/BGI/scratch/mweynants/DeepExtremes/tmax_smoothed.zarr")
...
# savefig(p1,"/Net/Groups/BGI/scratch/mweynants/DeepExtremes/map_t.png")
# p1 = heatmap(lons,lats,im1,title="Daily max t2m normalised")
# savefig(p1,"/Net/Groups/BGI/scratch/mweynants/DeepExtremes/map_t_normalised.png")
# p2 = heatmap(lons,lats,im2,title="Daily max t2m smoothed")
# savefig(p2,"/Net/Groups/BGI/scratch/mweynants/DeepExtremes/map_t_smoothed.png")
...
# p = plot(spec[2:end],yaxis=:log,xlabel="Wavenumber l",ylabel="Power",yrange=[1e-6,0.0],label="Unaltered Spectrum")
# plot!(p,clamp.(scalef.(1:720),1e-6+1e-7,1),yaxis=:log,label="Filter function")
# plot!(p,spec[2:end].*scalef.(1:720),label="Filtered Spectrum",title="Spherical Spectrum")
# savefig(p,"/Net/Groups/BGI/scratch/mweynants/DeepExtremes/spei_spectrum.png")

# imfiltered = SphericalConvolutions.lowpass(Float64.(im1),lbord = lbord,width=width)
