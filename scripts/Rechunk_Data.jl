...
# @everywhere 
function era5varcube(varname;ext="an")
    ...
    # remove from overall properties:
    delete!(props, "add_offset")
    delete!(props, "scale_factor")

    # cfargs = (onenc.data.mv,onenc.data.add_offset,onenc.data.scale_factor)
    spatsize = (length(lonax),length(latax))

    settings = (;
        varname,lonax, latax, T_CF, T_NC, spatsize#, props, cfargs
    )
    ...
    timax = Dim{:Ti}(DateTime(startyear,1,1):Hour(1):DateTime(endyear,12,31,23,0,0))
...
# @everywhere 
function diskarfromyearmonth(settings,y,m)
    ...
    # need to open file with cube to get properties
    onenc = Cube(fnow)
    cfargs = (onenc.data.mv,onenc.data.add_offset,onenc.data.scale_factor)

    # check that data array has the right size
    nts = length(onenc.Ti)
    nlons = length(onenc.longitude)
    nlats = length(onenc.latitude)
...
map(to_run) do (varname,outvarname,ext,aggfun)
    ...
    # set time axis 
    c2 = YAXArray((c.longitude,c.latitude,Dim{:time}( Date(startyear,1,1):Day(1):Date(endyear,12,31))),a2,c.properties)
    ...
    savedataset(ds, path="/Net/Groups/BGI/work_2/scratch/mweynants/Dheed_v4/ERA5Cube.zarr",append=true, max_cache=3e8,writefac=0.1)
    nothing
end
