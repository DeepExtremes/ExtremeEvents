...
@everywhere function era5varcube(varname;ext="an")
    onenc = Cube("/Net/Groups/data_BGC/era5/e1/0d25_hourly/$varname/1950/$(varname).hh.$(ext).era5_back_extension.01.1950.nc")
    ...
    allars = [diskarfromyearmonth(settings,y,m) for m in 1:12, y in 1950:2021]

    onear = ConcatDiskArray(reshape(allars,1,1,length(allars)))
    timax = RangeAxis("time",DateTime(1950,1,1):Hour(1):DateTime(2021,12,31,23,0,0))
...
map(to_run) do (varname,outvarname,ext,aggfun)
    ...
    c2 = YAXArray([c.longitude,c.latitude,RangeAxis("time",Date(1950,1,1):Day(1):Date(2021,12,31))],a2,c.properties)

    nt = (Symbol(outvarname)=>c2,)
    ds = Dataset(;nt...)
    ds = setchunks(ds,(lon=60,lat=60,time=5844))

    savedataset(ds, path="/scratch/fgans/XAIDAERA5Cube.zarr",append=true, max_cache=3e8,writefac=0.1)
    nothing
end
